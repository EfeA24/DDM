...
{
    public sealed class SqlServerReadAdapter : IReadProviderAdapter
    {
        private readonly SqlBuilder _sqlBuilder;

        public SqlServerReadAdapter(SqlBuilder sqlBuilder)
        ...
        public async Task<QueryResult> ExecuteAsync(string connectionString, ODataSpec spec, CancellationToken cancellationToken)
        {
            ...
            var selectStatement = _sqlBuilder.BuildSelect(spec, Dialect.SqlServer);
            ...
            if (spec.Count)
            {
                var countStatement = _sqlBuilder.BuildCount(spec, Dialect.SqlServer);
